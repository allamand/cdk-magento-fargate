...
This variable can be exported in your environment before creating the stack.
Other resources, can also be created based on the stack name, but you can also control this using the CDK context parameters through **Projen** configuration file, see below:
...
- **useEFS** yes/no. if yes, map /bitnami/magento directory to the EFS created file syste; (@default: no)
